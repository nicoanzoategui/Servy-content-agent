...
export async function runDailyFinanceSnapshot(): Promise<void> {
  ...
  try {
    ...
    await redis.setJson("finance:alerts:unresolved", { count: unresolved }, 3600);

    await insertAgentLog({
      agent: "finance",
      event: "daily_snapshot",
      level: "info",
      details: { periodKey, netRev, n },
...
export async function runWeeklyReport(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      geminiSummary = await geminiGenerateText(sys, "Generá el resumen ahora.");
    } catch (e) {
      geminiSummary = "(Resumen IA no disponible)";
      await insertAgentLog({
        agent: "finance",
        event: "weekly_gemini_skipped",
        level: "warn",
        details: { error: e instanceof Error ? e.message : String(e) },
      });
    }
    ...
    await WhatsAppService.sendToFounder(waBody);
...
export async function runProjections(): Promise<void> {
  ...
  try {
    ...
    const parsed = await geminiGenerateJson<ProjectionJson>(
      system,
    ...
    await prisma.financeProjection.create({
      data: {
        ...
        assumedCommissionRate:
          ...
          : 0.12,
        assumedMpFeeRate: env.MP_FEE_RATE,
    ...
    if (parsed.scenario_pessimist_ars < 0) {
      ...
      await WhatsAppService.sendToFounder(
